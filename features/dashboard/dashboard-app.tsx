  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="content-wrap">
        ...
        /> : <>
          {section === 'dashboard' && <DashboardOverview summary={summary} customers={customers} contracts={contractViews} payments={payments} today={today} onPay={setPaymentContract} onNavigate={go} />}
    ...
    {contractModal !== undefined && <ContractFormDialog value={contractModal} customers={customers.filter((customer) => !customer.archived)} defaults={settings} today={today} onClose={() => setContractModal(undefined)} onSaved={async (saved) => { setContractModal(undefined); setContracts((current) => contractModal.contract ? current.map((contract) => contract.id === saved.id ? saved : contract) : [...current, saved]); setSelectedContractId(saved.id); setSelectedCustomerId(saved.customer_id); setSection('customers'); notify(contractModal.contract ? 'تم تحديث العقد' : 'تمت إضافة العقد'); await load(true); }} />}
