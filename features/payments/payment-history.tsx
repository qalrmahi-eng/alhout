...
export default function PaymentHistory(props: Props) {
  ...
  return <section className="panel overflow-hidden">
    ...
    {props.payments.length ? <div className="table-wrap mt-5"><table><thead><tr><th>رقم الوصل</th><th>العميل / العقد</th><th>المبلغ</th><th>التاريخ</th><th>الملاحظة</th><th>الحالة</th><th>إجراء</th></tr></thead><tbody>{[...props.payments].reverse().map((payment) => {
      ...
      return <tr key={payment.id} className={cancelled ? 'cancelled-row' : ''}>
        ...
        <td className="font-black">{formatIqd(payment.amount)}</td><td>{formatDate(payment.payment_date)}</td><td>{payment.notes || '—'}</td>
        <td>{cancelled ? <span><b className="status status-archived">ملغاة</b><small className="block mt-1">{payment.cancellation_reason} · {formatBaghdadDateTime(payment.cancelled_at)}</small></span> : <b className="status status-paid">نشطة</b>}</td>
        <td><div className="flex gap-2"><button className="mini-button" onClick={() => props.onReceipt(payment)}>الوصل</button>{!cancelled && <button className="mini-button danger" onClick={() => props.onCancel(payment)}>إلغاء</button>}</div></td>
      </tr>;
