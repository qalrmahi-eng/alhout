...
import { addInstallmentPeriod, calculateContract, splitInstallments } from '@/lib/finance';
  ...
  const preview = useMemo(() => {
    try {
      ...
      return {
        ...calculated,
        installment: parts[0],
        expectedEnd: addInstallmentPeriod(firstDueDate, installments - 1, 'monthly'),
      };
    } catch { return null; }
  }, [principal, profitPercent, installments, firstDueDate]);
  ...
  return <div className="modal-backdrop"><div className="modal-panel max-w-4xl"><ModalHeader title={contract ? `تعديل العقد C-${contract.id}` : 'إضافة عقد جديد'} onClose={props.onClose} />
    <div className="contract-form-layout">
      <form className="form-grid" onSubmit={submit}>
        ...
        <Field label="عدد الأشهر"><input name="installments" type="number" min="1" step="1" value={installments} onChange={(event) => setInstallments(Number(event.target.value))} required /></Field>
        ...
        <Field label="تاريخ أول استحقاق"><input name="first_due_date" type="date" value={firstDueDate} onChange={(event) => setFirstDueDate(event.target.value)} required /></Field>
