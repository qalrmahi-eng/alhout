...
export default function ManualReminderDialog({ contract, onClose, onSaved }: Props) {
  ...
  return <div className="modal-backdrop"><div className="modal-panel max-w-md">
    <ModalHeader title={contract.manual_reminder_date ? 'تعديل موعد التذكير' : 'تحديد موعد التذكير'} onClose={onClose} />
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="modal-actions flex-wrap">
        {contract.manual_reminder_date && <button type="button" className="secondary-button" disabled={saving} onClick={clear}>العودة للتذكير التلقائي</button>}
