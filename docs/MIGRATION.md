...
الترقية توسعية: تحفظ أسماء الأعمدة القديمة وترتيبها، وتضيف الأعمدة الجديدة إلى اليمين فقط. العملاء القدامى الذين لا يملكون `installment_type` يعاملون شهريًا، ويُستخدم `start_date` كقيمة عرض بديلة لتاريخ تسليم المبلغ فقط. يبقى `first_due_date` مستقلاً، وتنبّه الواجهة إلى أن البيانات الناقصة تحتاج مراجعة.
